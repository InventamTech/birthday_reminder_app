...
import { ADD_FRIEND, BULK_ADD_FRIEND, DELETE_FRIEND, EDIT_FRIEND, GET_FRIEND } from '../actions';
import { closeModal } from '../dashboard/actions';
import {
	addFriendError,
	addFriendSuccess,
	bulkAddFriendError,
	bulkAddFriendSuccess,
	deleteFriendError,
	deleteFriendSuccess,
	editFriendError,
	editFriendSuccess,
	...
	getFriendSuccess,
	selectFriend,
	getFriend as getFriendDispatch
} from './actions';
...
export function* watchBulkAddFriend() {
	yield takeEvery(BULK_ADD_FRIEND, bulkAddFriend);
}

function* bulkAddFriend({ payload }) {
	const {
		files
	} = payload
	try {
		...
		const bulkResponse = yield axios.post(`/friend-csv`, data, getHeader())
		// console.log(bulkResponse)
		// // console.log('edit friend:', editFriendResponse.data)
		// // yield put(editFriendSuccess({ name, email, dob }, id));
		yield put(bulkAddFriendSuccess());
		yield put(closeModal());
		yield put(selectFriend(undefined));
		yield put(getFriendDispatch());
	} catch (error) {
		const message =
			(error.response && error.response.data.message) || 'Something went wrong!';
		console.log('error while delete friend:', error);
		yield put(bulkAddFriendError(message));
	}
}
